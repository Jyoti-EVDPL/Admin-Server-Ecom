...
const DeliveryAddProcView = async (info) => {
    const data2 = await UserRepository.SignIn({username:info});
    const data = await UserRepository.DeliveryAddView(data2.dataValues.customer_id);
    if (!data) throw new ApiError(400, "Saved Address Not found")
    return data;
    // return { message: "Saved Address found" };
}
const DeliveryAddProcAdd = async (info) => {
    const data = await UserRepository.DeliveryAddAdd(info);
    if (!data.dataValues) throw new ApiError(400, "Unable to Add Delivery Address")
    return { message: "User Add Delivery Address successfull" };
}
const DeliveryAddProcEdit = async (info) => {
...
const DeliveryAddProcDelete = async (info) => {
    const { password, ...rest } = info;
    const hashedPassword = await bcrypt.hash(password, 10);
    const userData = { ...rest, password: hashedPassword };
    const data = await UserRepository.SignUp(userData);
...
//To View SavedAddresses
const SavedAddressViewProcess = async (username) => {
    const data = await UserRepository.SavedAddressViewUser(username);
    // if (data) {
    //     console.log("Data not added")
    // } else {
    //     console.log("New User Created successfully")
...
//To Add SavedAddresses
const SavedAddressAddProcess = async (role_name, role_isactive, role_desc, claim) => {
    const data = await UserRepository.SavedAddressAddUser(role_name, role_isactive, role_desc, claim);
    if (!data) {
...
//To Add SavedAddresses
const SavedAddressEditProcess = async (role_id, role_name, role_isactive, role_desc, claim) => {
    const data = await UserRepository.SavedAddressEditUser(role_id, role_name, role_isactive, role_desc, claim);
    if (!data) {
        console.log("Data not found")
        return null;
    } else {
...
//To Delete SavedAddresses
const SavedAddressDeleteProcess = async (username, role_id) => {
    const message = await UserRepository.SavedAddressDeleteUser(username, role_id);
    if (message) {
...
//VIEW PRODUCTS
const ViewProductsProcess = async () => {
    const data = await UserRepository.ViewProducts();
    if (!data) {
        console.log("Products Data not found")
        return null;
    } else {
        console.log("Products Data found successfully")
        return data;
    }
    // return data;
}
...
module.exports = {
    ...
    SavedAddressViewProcess,
    SavedAddressAddProcess,
    SavedAddressEditProcess,
    SavedAddressDeleteProcess,
    //..//Role Management
    ViewRoleProcess,
    ViewClaimProcess,
    AddRoleProcess,
