...
const authServices = require("../service/authenticationService");

//-------------------PROTECTED ROUTES-----------------------------------
//User Delivery Address Route
const DeliveryAddView = async (req, res, next) => {
    try {
        const data = await authServices.DeliveryAddProcView(req.params.username);
        res.send(data)
    } catch (e) { next(e) }
};
const DeliveryAddAdd = async (req, res, next) => {
    try {
        const data = await authServices.DeliveryAddProcAdd(req.body);
        res.send(data)
    } catch (e) { next(e) }
};
const DeliveryAddEdit = async (req, res, next) => {
    try {
        const data = await authServices.DeliveryAddProcEdit(req.body);
        res.send(data)
    } catch (e) { next(e) }
};
const DeliveryAddDelete = async (req, res, next) => {
    try {
        const data = await authServices.DeliveryAddProcDelete(req.body);
        res.send(data)
    } catch (e) { next(e) }
};
...
//SavedAddress Add
const SavedAddressView = async (res, req) => {
    const { username } = res.body;
    console.log(username);
    const data = await authServices.SavedAddressViewProcess(username)
    req.send({ data });
    // req.send('User Created');
};
//SavedAddress Add
const SavedAddressAdd = async (res, req) => {
    ...
    // req.send(data);
    req.status(200);
    req.send({ message: "Created" });
};
//SavedAddress Edit
const SavedAddressEdit = async (res, req) => {
...
//SavedAddress Delete
const SavedAddressDelete = async (res, req) => {
...
//-------------------------------------------------------------------------------------------
//PRODUCTS LIST
const ProductsView = async (res, req) => {
    const { username } = res.body;
    // console.log(username);
    const data = await authServices.ViewProductsProcess()
    req.send(data);
    req.status(200);
    // req.send('User Created');
};
